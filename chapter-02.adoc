...
| center | A center point for the map image. | 40.039401,-76.307078 | Yes, if the bestfit parameter is not present and no POIs are present
| zoom | Specifies zoom level for the map image. | A valid zoom integer. See http://open.mapquestapi.com/staticmap/zoomToScale.html[Zoom to Scale Mapping] for information on scales versus zoom levels. | Yes, if the center parameter is present, the scale parameter is not present, and no POIs are present  
